...
import { MapPin, Navigation, Home, Package, Truck, Maximize2, Minimize2 } from 'lucide-react';
...
export const LiveKitchenMap: React.FC<LiveKitchenMapProps> = ({
  ...
  const [selectedMarker, setSelectedMarker] = useState<'chef' | 'delivery' | 'agent' | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (chefLocation?.latitude && chefLocation?.longitude) {
        bounds.extend(new google.maps.LatLng(chefLocation.latitude, chefLocation.longitude));
        hasLocations = true;
      }

      // Add delivery location
      if (deliveryLocation?.latitude && deliveryLocation?.longitude) {
        bounds.extend(new google.maps.LatLng(deliveryLocation.latitude, deliveryLocation.longitude));
        hasLocations = true;
      }

      // Add agent location
      if (agentLocation?.latitude && agentLocation?.longitude) {
        bounds.extend(new google.maps.LatLng(agentLocation.latitude, agentLocation.longitude));
        hasLocations = true;
      }
  ...
  const getMapCenter = () => {
    ...
    if (deliveryLocation && deliveryLocation.latitude && deliveryLocation.longitude) {
      return { lat: deliveryLocation.latitude, lng: deliveryLocation.longitude };
    }
  ...
  const getPolylinePath = () => {
    ...
    if (deliveryLocation) {
      path.push({ lat: deliveryLocation.latitude, lng: deliveryLocation.longitude });
    }
  ...
  if (!isLoaded && !loadError) {
    return (
      <div className={cn('w-full h-full bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 flex items-center justify-center rounded-lg', className)}>
        <div className="text-center">
          ...
          <p className="text-gray-700 dark:text-gray-300 font-semibold text-sm">Loading map...</p>
          <p className="text-gray-500 dark:text-gray-400 text-xs mt-1">Please wait</p>
        </div>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className={cn('rounded-lg bg-gray-100 dark:bg-gray-800 p-8 text-center', className)}>
  ...
  return (
    <div ref={mapRef} className={cn('relative w-full h-full overflow-hidden bg-gray-100 dark:bg-gray-800 rounded-lg', className)}>
        <GoogleMap
        ...
        >
          ...
          {chefLocation && chefLocation.latitude && chefLocation.longitude && (
            <>
              <Marker
                position={{ lat: chefLocation.latitude, lng: chefLocation.longitude }}
                icon={{
              ...
              <InfoWindow
                position={{ lat: chefLocation.latitude, lng: chefLocation.longitude + 0.0015 }}
                options={{
                  pixelOffset: new (window.google?.maps?.Size || class {})(0, -50),
                  disableAutoPan: true,
                  closeBoxURL: '',
                }}
              >
                <div className="bg-white dark:bg-gray-800 px-2 py-1 rounded-md shadow-lg border-2 border-orange-500">
                  <p className="text-xs font-bold text-orange-600 dark:text-orange-400">🍳 Kitchen</p>
                </div>
              </InfoWindow>
              ...
              {selectedMarker === 'chef' && (
                ...
                >
                  <div className="p-2">
                    ...
                    <p className="text-sm text-gray-600">{chefLocation.address}</p>
                  </div>
                </InfoWindow>
              )}
            </>
          ...
          {deliveryLocation && (
            <>
              <Marker
                ...
                icon={{
                  url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#10b981" stroke="#ffffff" stroke-width="2"/></svg>')}`,
                }}
          ...
          {agentLocation && orderStatus === 'out_for_delivery' && (
            <>
              <Marker
                ...
                icon={{
                  url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#3b82f6" stroke="#ffffff" stroke-width="2"/><path d="M8 12 L12 8 L16 12 L12 10 Z" fill="#ffffff"/></svg>')}`,
                }}
