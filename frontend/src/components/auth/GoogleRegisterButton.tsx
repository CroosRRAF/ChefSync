...
const GoogleRegisterButton: React.FC<GoogleAuthButtonProps> = ({ 
  mode = 'register',
  onSuccess 
}) => {
  ...
  // If no valid Google client ID, show a disabled button with helpful message
  if (!isValidClientId) {
    return (
      <Button
        variant="outline"
        ...
        onClick={() => {
          toast({
            ...
            description: "Google authentication is not configured. Please use email/password to sign up.",
            duration: 5000,
          });
        }}
      >
  ...
  return (
    <GoogleLogin
      ...
      onError={() => toast({ variant: 'destructive', title: 'Google authentication failed', description: 'Please try again or use email/password' })}
    />
  );
};
