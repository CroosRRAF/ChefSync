...
import { useDebounce, useVirtualScrolling } from '@/utils/performance';
...
const MemoizedDataTable: React.FC<MemoizedDataTableProps> = memo(({
  ...
  React.useEffect(() => {
    ...
    if (renderTime > 100) { // Log slow renders
      console.warn(`🐌 Slow DataTable render: ${renderTime.toFixed(2)}ms for ${data.length} items`);
    }
...
export const areDataTablePropsEqual = (
  ...
  return (
    ...
    JSON.stringify(prevProps.bulkActions) === JSON.stringify(nextProps.bulkActions)
  );
};

// Highly optimized version with custom comparison
export const HighlyOptimizedDataTable = memo(MemoizedDataTable, areDataTablePropsEqual);
