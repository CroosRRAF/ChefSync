...
const EnhancedAdminLayout: React.FC<AdminLayoutProps> = memo(({ children }) => {
    ...
    return (
      <div key={item.href} className="space-y-1">
        <Button
          ...
          className={cn(
            "w-full justify-start gap-3 text-left font-normal h-auto py-3 px-3",
            level > 0 && "ml-6 w-[calc(100%-1.5rem)]",
            sidebarCollapsed && !level && "justify-center px-2",
            isActive && "border-r-2 border-primary bg-primary/10"
          )}
          onClick={() => {
            navigate(item.href);
  ...
  }, [isActiveRoute, expandedItems, sidebarCollapsed, navigate]);
  ...
  return (
    <div className="h-screen flex bg-background">
      {/* Desktop Sidebar */}
      <aside className={cn(
        "hidden lg:flex flex-col border-r transition-all duration-300 bg-card",
        ...
        <header className="h-16 border-b flex items-center justify-between px-4 lg:px-6 bg-card">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex items-center">
              {/* Search Bar */}
              {searchOpen && (
                <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-2 rounded-lg shadow-lg border bg-card px-3 py-2 w-80 animate-in slide-in-from-top-2">
