...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Generating test data for {days} days...'
            )
        )
        ...
        with transaction.atomic():
            ...
            self.create_orders_over_time(
                days, orders_per_day, customers, chefs, foods
            )

        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully generated test data for {days} days!'
            )
        )
