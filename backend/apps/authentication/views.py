    ...
    if serializer.is_valid():
        ...
        if referral_token:
            ...
            if not referral_result["success"]:
                ...
                logger.error(
                    f"Referral token usage failed: {referral_result['message']}"
                )
...
def google_oauth_login(request):
    ...
    logger.info(
        f'CSRF exempt check: {getattr(request, "_dont_enforce_csrf_checks", False)}'
    )
    logger.info(f"Client ID from settings: {settings.GOOGLE_OAUTH_CLIENT_ID}")
    logger.info(
        f"Client Secret configured: {bool(settings.GOOGLE_OAUTH_CLIENT_SECRET)}"
    )
...
def upload_profile_image(request):
    """
    Upload profile image to Cloudinary with comprehensive error handling
    """
    ...
    try:
        ...
        if isinstance(image_data, str):
            if not image_data.startswith("data:image") and len(image_data) < 100:
                return Response(
        ...
        if estimated_size_mb > max_size_mb:
            return Response(
                {"error": f"Image size too large. Maximum {max_size_mb}MB allowed."},
                status=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            )

        logger.info(f"User {request.user.user_id} attempting profile image upload")
        ...
        try:
            cloudinary_result = upload_image_to_cloudinary(
                ...
                public_id=f"user_{request.user.user_id}_profile",
                tags=["profile", "user", str(request.user.user_id)],
            )
        ...
        if not cloudinary_result or "secure_url" not in cloudinary_result:
            logger.error(f"Invalid Cloudinary response: {cloudinary_result}")
            return Response(
                {"error": "Failed to upload image - invalid response from storage"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        # Update user's profile_image field with transaction safety
        try:
            with transaction.atomic():
                ...
                user.profile_image = cloudinary_result["secure_url"]
                user.save(update_fields=["profile_image"])

                logger.info(f"Profile image updated for user {user.user_id}")
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if user.role == "customer":
                ...
                print(
                    f"✅ No tokens provided for {user.role} - requires admin approval"
                )  # Debug log
                ...
                return Response(
                    {
                        ...
                        "message": (
                            errors["file_upload"][0]
                            if isinstance(errors["file_upload"], list)
                            else str(errors["file_upload"])
                        ),
                        "details": errors,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
...
def update_chef_location(request):
    ...
    try:
        ...
        return Response(
            {"error": "Cook profile not found"}, status=status.HTTP_404_NOT_FOUND
        )
    except Exception as e:
        return Response(
...
def toggle_location_tracking(request):
    ...
    try:
        ...
        return Response(
            {"message": message, "is_tracking": cook.is_location_tracking_enabled},
            status=status.HTTP_200_OK,
        )
