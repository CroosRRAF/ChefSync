...
class User(AbstractUser):
    ...
    def get_profile(self):
        ...
        if self.role in ["customer", "Customer"]:
            ...
            try:
                return Cook.objects.get(user=self)
            except Cook.DoesNotExist:
                return None
        elif self.role in ["delivery_agent", "DeliveryAgent"]:
            try:
                return DeliveryAgent.objects.get(user=self)
...
class DocumentType(models.Model):
    ...
    def __str__(self):
        category_display = dict(self.DOCUMENT_CATEGORIES).get(
...
class JWTToken(models.Model):
    ...
    def create_referral_token(
        ...
        # Generate unique JTI
        while True:
            jti = "".join(random.choices(string.ascii_letters + string.digits, k=32))
