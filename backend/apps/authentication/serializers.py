...
class UserDocumentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        file = validated_data.pop(
            "file_upload"
        )  # Remove file_upload from validated_data to avoid duplicate
        ...
        if is_pdf:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"Failed to save original PDF locally: {str(local_save_error)}"
                    )
            ...
            if use_local:
                ...
                if file_type and file_type.startswith("image/"):
                    ...
                    folder_name = (
                        "assets/documents/other"  # Default to other documents folder
                    )
                ...
                # Create local URL
                primary_file_url = (
                    f"/local_media/{folder_name}/{user.user_id}/{unique_filename}"
                )
                cloudinary_public_id = None
            else:
                ...
                try:
                    ...
                    if resource_type == "raw":
                        try:
                            ...
                            file_extension = (
                                file_name.split(".")[-1] if "." in file_name else "pdf"
                            )
                            unique_filename = f"{uuid.uuid4()}_{file_name}"
                            local_file_path = local_folder / unique_filename
                            ...
                            print(
                                f"Failed to save PDF locally: {str(local_save_error)}"
                            )
        ...
        document = UserDocument.objects.create(
            ...
            local_file_path=(
                local_file_path_str if "local_file_path_str" in locals() else None
            ),
