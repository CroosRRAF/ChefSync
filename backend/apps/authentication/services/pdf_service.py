...
class PDFService:
    ...
    def _find_poppler_path(self):
        ...
        logger.warning(
            "Poppler not found in common locations. pdf2image will try to find it automatically."
        )
        return None
        ...
        try:
            ...
            if page_count is not None and page_count > self.MAX_PAGES:
                raise PDFValidationError(
                    f"PDF has {page_count} pages. Maximum allowed is {self.MAX_PAGES} pages."
                )
    ...
    def convert_pdf_to_images(
        self, file: UploadedFile, user_email: str, document_type_name: str
    ) -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            if len(images) > self.MAX_PAGES:
                raise PDFConversionError(
                    f"PDF has {len(images)} pages. Maximum allowed is {self.MAX_PAGES} pages."
                )
            ...
            for i, image in enumerate(images):
                try:
                    ...
                    logger.info(
                        f"Successfully uploaded page {i+1} to Cloudinary: {upload_result['public_id']}"
                    )
            ...
            logger.info(
                f"Successfully converted and uploaded {len(uploaded_images)} pages for user: {user_email}"
            )
    ...
    def validate_and_convert_pdf(
        ...
        try:
            ...
            converted_images = self.convert_pdf_to_images(
                file, user_email, document_type_name
            )
...
def convert_pdf_to_images(
    file: UploadedFile, user_email: str, document_type_name: str
) -> List[Dict[str, str]]:
    """
