...
from apps.authentication.models import JWTToken, User
...
class JWTTokenService:
    ...
    @classmethod
    def validate_token(
        cls, token: str, token_type: str = "access"
            ...
            if token_type == "access":
                ...
                try:
                    token_record = JWTToken.objects.get(
                        token_hash=token_hash, token_type="refresh", user=user
                    )
    ...
    def refresh_access_token(cls, refresh_token: str, request=None) -> Dict[str, Any]:
        ...
        # Generate new access token using RefreshToken.for_user
        refresh = RefreshToken.for_user(user)
        access = refresh.access_token
    ...
    def revoke_all_user_tokens(
        cls, user: User, token_type: Optional[str] = None
    ) -> int:
        """
    ...
    @classmethod
    def get_token_info(
        cls, token: str, token_type: str = "refresh"
