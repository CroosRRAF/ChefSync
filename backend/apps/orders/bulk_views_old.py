...
from .models import Order, OrderItem, OrderStatusHistory, BulkOrder, BulkOrderMenuItem, BulkOrderCollaboration
from .serializers import (
    OrderDetailSerializer, OrderListSerializer, BulkOrderActionSerializer,
    BulkOrderListSerializer, BulkOrderDetailSerializer, BulkOrderCreateSerializer,
    BulkOrderUpdateSerializer, BulkOrderCollaborationRequestSerializer, BulkOrderStatsSerializer
)
...
class BulkOrderManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            ...
            return BulkOrderUpdateSerializer
        return BulkOrderDetailSerializer
    
    def get_queryset(self):
        """Get bulk orders with filtering"""
        queryset = BulkOrder.objects.select_related('customer', 'primary_chef').prefetch_related(
            'collaborating_chefs', 'menu_items', 'collaborations'
        )
        ...
        if search:
            queryset = queryset.filter(
                Q(bulk_order_number__icontains=search) |
    ...
    def accept(self, request, pk=None):
        ...
        bulk_order.chef_response = notes
    ...
    def decline(self, request, pk=None):
        ...
        if bulk_order.status not in ['pending', 'quote_requested']:
            return Response(
                {'error': 'Only pending or quote requested bulk orders can be declined'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        
        bulk_order.status = 'cancelled'
        bulk_order.chef_response = reason
        bulk_order.cancelled_at = timezone.now()
        bulk_order.save()
    ...
    def collaborate(self, request, pk=None):
        ...
        bulk_order = self.get_object()
        serializer = BulkOrderCollaborationRequestSerializer(data=request.data)
        
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        
        chef_id = serializer.validated_data['chef_id']
        
        try:
            collaborating_chef = ChefProfile.objects.get(id=chef_id).user
        ...
        if bulk_order.collaborations.filter(chef=collaborating_chef).exists():
            return Response(
                {'error': 'Collaboration with this chef already exists'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Create collaboration
        collaboration = BulkOrderCollaboration.objects.create(
            bulk_order=bulk_order,
            chef=collaborating_chef,
            role=serializer.validated_data['role'],
            responsibilities=serializer.validated_data['responsibilities'],
            work_percentage=serializer.validated_data['work_percentage'],
            agreed_amount=serializer.validated_data['agreed_amount'],
            invitation_message=serializer.validated_data.get('invitation_message', ''),
            status='invited'
        )
        
        # Update bulk order collaboration status
        bulk_order.collaboration_status = 'requesting'
        bulk_order.save()
    ...
    def stats(self, request):
        ...
        total_revenue = BulkOrder.objects.filter(
            ...
            total=Sum('quoted_amount')
        )['total'] or 0
        
        this_month_revenue = BulkOrder.objects.filter(
            status__in=['completed', 'delivered'],
        ...
        upcoming_events = BulkOrder.objects.filter(
        ...
        active_collaborations = BulkOrderCollaboration.objects.filter(
            status='accepted',
            bulk_order__status__in=['confirmed', 'preparing']
        ).count()
