...
class Command(BaseCommand):
    ...
    def generate_orders_and_payments(self, data_months):
        ...
        for i in range(total_orders):
            ...
            discount = Decimal(str(round(random.uniform(0, subtotal * Decimal('0.2')), 2))) if random.choice([True, False, False]) else Decimal('0.00')
            total_amount = subtotal + tax_amount + order.delivery_fee - discount
    ...
    def generate_admin_data(self, data_months):
        ...
        for i in range(1000):  # Generate 1000 activity logs
            ...
            log = AdminActivityLog.objects.create(
                ...
                target_model=random.choice([
                    'User', 'Order', 'Payment', 'Food', 'Cuisine', 'AdminNotification'
                ]),
        ...
        for i in range(data_months * 30):  # Daily metrics
            ...
            metric = SystemHealthMetric.objects.create(
                ...
                severity='low' if cpu_usage < 70 else 'medium' if cpu_usage < 85 else 'high'
            )
            metrics.append(metric)

        # Generate admin notifications
        notification_types = [
            'system_alert', 'order_notification', 'payment_notification',
            'user_notification', 'security_alert', 'maintenance_notification'
        ]
        ...
        for i in range(200):  # Generate 200 notifications
            ...
            notification = AdminNotification.objects.create(
                ...
                target_users=random.sample(self.admin_users, random.randint(1, len(self.admin_users))),
        ...
        widgets = []
        widget_configs = [
            {'name': 'Total Orders', 'type': 'metric', 'config': {'value': len(self.orders), 'format': 'number'}},
            {'name': 'Total Revenue', 'type': 'metric', 'config': {'value': sum(o.total_amount for o in self.orders), 'format': 'currency'}},
            {'name': 'Active Users', 'type': 'metric', 'config': {'value': len(self.customer_users), 'format': 'number'}},
            {'name': 'System Health', 'type': 'chart', 'config': {'type': 'line', 'data': 'cpu_usage'}},
            {'name': 'Recent Orders', 'type': 'table', 'config': {'limit': 10, 'sort': '-created_at'}},
            {'name': 'Popular Foods', 'type': 'chart', 'config': {'type': 'bar', 'data': 'order_count'}},
        ]
        ...
        actions = []
        quick_actions = [
            {'name': 'Create New Order', 'action_type': 'create', 'target_model': 'Order', 'icon': 'plus'},
            {'name': 'Process Refund', 'action_type': 'update', 'target_model': 'Payment', 'icon': 'refund'},
            {'name': 'Send Notification', 'action_type': 'create', 'target_model': 'AdminNotification', 'icon': 'bell'},
            {'name': 'Generate Report', 'action_type': 'export', 'target_model': 'Order', 'icon': 'download'},
            {'name': 'Update System Settings', 'action_type': 'update', 'target_model': 'AdminSystemSettings', 'icon': 'settings'},
        ]
        ...
        for action_data in quick_actions:
            action = AdminQuickAction.objects.create(
                ...
                configuration={'icon': action_data['icon'], 'color': random.choice(['primary', 'success', 'warning', 'danger'])},
                is_active=True,
                created_by=random.choice(self.admin_users),
                created_at=timezone.now() - timedelta(days=random.randint(1, 30))
            )
        ...
        for i in range(30):  # Monthly backups for the past year
            ...
            backup = AdminBackupLog.objects.create(
                backup_name=f'backup_{backup_date.strftime("%Y%m%d")}',
                ...
                initiated_by=random.choice(self.admin_users),
                ...
                updated_at=backup_date + timedelta(minutes=random.randint(30, 120))
            )
